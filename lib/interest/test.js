...
const testCompoundResolverSimple = () => {
  let rest = Rounder(5);
  let resolve = createResolver(rest, 'simple');
  let compoundOutput = resolve({ seed: 1000, rate: 0.05, runtime: 2 });
  let rate = resolve({ seed: 1000, output: compoundOutput, runtime: 2 });
  let runtime = resolve({ seed: 1000, output: compoundOutput, rate: 0.05 });
  let seed = resolve({ runtime: runtime, output: compoundOutput, rate: rate });
...
const testFinancing = () => {
  console.log(interest.financing(0.12/12, 500, 1));
  console.log(interest.pension(0.12/12, 500, 1));
};
...
// testCompoundResolverSimple();
testFinancing();
// console.log(interest.financing(0.12/12, 500, 12));
// console.log(interest.pension(0.12/12, 500, 12));
